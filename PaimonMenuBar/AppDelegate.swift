...
final class AppDelegate: NSObject, NSApplicationDelegate {
    ...
    func updateStatusBar() {
        ...
        let iconStyle = Defaults[.lastIconStyle]
        if gameRecord.fetchAt == nil {
            statusButton.title = "-/160" // Cookie Not configured
        } else {
            switch(iconStyle){
            case .Normal:statusButton.title = "\(gameRecord.data.current_resin)/\(gameRecord.data.max_resin)";
            case .Compact: statusButton.title = "\(gameRecord.data.current_resin)";
            case .IconOnly: statusButton.title = ""
            }
        }
